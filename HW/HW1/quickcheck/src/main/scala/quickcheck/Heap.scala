...
trait BinomialHeap extends Heap {
  ...
  protected def link(t1: Node, t2: Node): Node = {
    //println("link:: Node 1: "+t1.x+", Node 2: "+t2.x)// t1.r==t2.r
  ...
  protected def ins(t: Node, ts: H): H = ts match {
    case Nil => 
      ...
      List(t)
    case tp::ts => 
      //val j = println("ins:: t.r: "+t.r+", tp.r: "+tp.r)// t.r<=tp.r
      if (t.r<tp.r) {
        val joined = t::tp::ts 
        println("ins:: Joined: "+joined)
        joined
      }
      else {    
        val l = link(t,tp)
        //println("ins:: linked: "+l)
        //println("ins:: ts: "+ts)
        //ins(link(t, tp), ts)
        ins(l, ts)
      }
  ...
  override def insert(x: A, ts: H) = {
    println("Insert begin, insert x: "+x)
    val r = ins(Node(x,0,Nil), ts)
    println("Insert end, heap: "+r)
    r
  }
